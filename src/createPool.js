(function createPool (n) {
  ...
  function nextJob (t) {
    ...
    if (job) {
      if (job.type === kTypeRun) {
        t.eval(job.srcTextOrEventType, job.buf, function idleCB (e, d) {
          nextJob(t);
          var f= job.cbOrData;
          if (f) {
  ...
  function qPush (srcTextOrEventType, cbOrData, type,  buf, dirname) {
    var job= { next:null, srcTextOrEventType:srcTextOrEventType, cbOrData:cbOrData, type:type };
	if(buf) job.buf = buf;
	if(dirname) job.dirname = dirname;
  ...
  function evalAny (src, buf, cb, dirname) {
    qPush(src, cb, kTypeRun, buf, dirname);
